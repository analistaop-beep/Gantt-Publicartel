...
const Services = {
    ...
    addProductionOrder: (order) => {
        const id = uuidv4();
        db.prepare('INSERT INTO production_orders (id, opNumber, client, seller, price, currency, description, address, category, status, files, soporte) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)')
            .run(id, order.opNumber, order.client, order.seller, order.price, order.currency, order.description, order.address, order.category, order.status, JSON.stringify(order.files || []), order.soporte || null);
        return id;
    },
    updateProductionOrder: (order) => {
        db.prepare('UPDATE production_orders SET opNumber = ?, client = ?, seller = ?, price = ?, currency = ?, description = ?, address = ?, category = ?, status = ?, files = ?, soporte = ? WHERE id = ?')
            .run(order.opNumber, order.client, order.seller, order.price, order.currency, order.description, order.address, order.category, order.status, JSON.stringify(order.files || []), order.soporte || null, order.id);
    },
